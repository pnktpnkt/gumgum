...
public class Turtle_AnimationEvents : MonoBehaviour
{
	...
	void Start()
	{
		shootingTarget = GameObject.FindGameObjectWithTag ("Player").transform;
		/*
		* ANIMATION EVENTS
		* */
		// MINING ANIMATION
	...
	void AnimEvent(string val)
	{
		...
		Vector3 direction = (shootingTarget.position - positions[int.Parse (vals[1])].position).normalized;
		Quaternion emitRotation = Quaternion.LookRotation (direction);
		Transform t = Instantiate(particles[materialType].list[int.Parse (vals[0])], positions[int.Parse (vals[1])].position, emitRotation);
